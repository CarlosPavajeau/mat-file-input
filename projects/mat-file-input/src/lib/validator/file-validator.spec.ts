...
import { maxContentSize } from './file-validator';

describe('FileValidator', () => {
  describe('maxContentSize', () => {
    it('should validate', () => {
      const data = new FileInput([ new File([ 'test' ], 'test.txt') ]);
      const control = new FormControl(data, [ maxContentSize(5) ]);
    ...
    it('should validate with size equal', () => {
      const data = new FileInput([ new File([ 'test' ], 'test.txt') ]);
      const control = new FormControl(data, [ maxContentSize(4) ]);
      expect(control.value).toBe(data);
      expect(control.valid).toBeTruthy();
    });
    ...
    it('should not validate', () => {
      const data = new FileInput([ new File([ 'test' ], 'test.txt') ]);
      const control = new FormControl(data, [ maxContentSize(3) ]);
      expect(control.value).toBe(data);
    ...
    it('should validate with no files', () => {
      const control = new FormControl(undefined, [ maxContentSize(3) ]);
